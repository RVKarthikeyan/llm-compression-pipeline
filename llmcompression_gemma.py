...
# Load Teacher Model: Google Gemma 3 
import torch
...
print("="*70)
print("Loading Teacher Model: Google Gemma 3")
print("="*70)
...
# Load Gemma 3 Instruction-tuned model
...
try:
    ...
    # print(f"   Model: Google Gemma 3 12B (Instruction-tuned)")
    # print(f"   Parameters: ~12B (4-bit quantized)")
    # print(f"   Memory usage: ~8 GB VRAM")
    ...
    print("2. Request access at: https://huggingface.co/google/gemma-3-12b-it")
...
print(f"\n{'='*70}")
print(f"Ready to fine-tune student model (Gemma 3 1B)")
print(f"{'='*70}")
...
try:
    ...
    print("Teacher model (Gemma 3 4B) cleared from GPU memory")
except Exception:
    print("No model to clear")

print("\n" + "="*70)
print("Loading Student Model: Google Gemma 3 1B")
print("="*70)

# Load Gemma 3 1B student model
gemma3_model_name = "google/gemma-3-1b-it"

print(f"\nLoading {gemma3_model_name}...")
print("   Initial load may take several minutes...")

try:
    # Load with 4-bit quantization for memory efficiency
    gemma3_tokenizer = AutoTokenizer.from_pretrained(gemma3_model_name, token=True)
    ...
    gemma3_tokenizer.pad_token = gemma3_tokenizer.eos_token
    gemma3_tokenizer.padding_side = "right"

    print("Student model loaded successfully")
    print(f"\nModel Specifications:")
    print(f"   Model: {gemma3_model_name}")
    ...
    print("2. Request access at: https://huggingface.co/google/gemma-3-1b-it")
...
# Format conversations for Gemma chat template
def format_conversation_gemma(example):
    """
    Format conversation using Gemma's specific chat template.
    Template: <start_of_turn>user\n{prompt}<end_of_turn>\n<start_of_turn>model\n{response}<end_of_turn>
    """
    messages = example["messages"]
    formatted = gemma3_tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=False)
    return {"text": formatted}

# Convert to Hugging Face Dataset format
print("\nConverting to Hugging Face Dataset format...")
dataset = Dataset.from_list(chat_data)
dataset = dataset.map(format_conversation_gemma)
...
gemma3_model = prepare_model_for_kbit_training(gemma3_model)
...
# Apply LoRA to model
gemma3_model = get_peft_model(gemma3_model, lora_config)

# Print trainable parameters
trainable_params = sum(p.numel() for p in gemma3_model.parameters() if p.requires_grad)
...
print(f"Model checkpoints will be saved to: ./gemma3-chat-domain/")

# Execute training process
print("="*70)
print("Starting Training: Gemma 3 1B Learning from Gemma 3 4B")
print("="*70)
...
final_model_path = "./gemma3-chat-final"
trainer.model.save_pretrained(final_model_path)
gemma3_tokenizer.save_pretrained(final_model_path)
...
# Inference function
def chat_with_gemma(user_message, max_length=512):
    """
    Generate response using fine-tuned Gemma 3 model.
...
for i, query in enumerate(test_queries, 1):
    ...
    response = chat_with_gemma(query)
    print(response)
...
try:
    ...
    print("\nLoading base Gemma 3 1B model (FP16)...")
    ...
    # Save merged model
    merged_path = "./gemma3-domain-specialist-merged"
...
try:
    result = subprocess.run([
        ...
        merged_path,
        "--outfile", "./gemma3-domain-specialist.gguf",
        "--outtype", "f16"
    ...
    if result.returncode == 0:
        ...
        gguf_file = "./gemma3-domain-specialist.gguf"
        if os.path.exists(gguf_file):
            gguf_size = os.path.getsize(gguf_file) / (1024 * 1024)
            print(f"\nModel Information:")
            print(f"   Filename: gemma3-domain-specialist.gguf")
    ...
    print(f"   python llama.cpp/convert_hf_to_gguf.py {merged_path} --outfile gemma3-domain-specialist.gguf --outtype f16")
...
# base_dir = "/content"
# drive_folder_path = "/content/drive/MyDrive/model-c-specific"
...
# artifacts_to_copy = [
#     "gemma3-chat-final",                     # The trained LoRA adapter
#     "gemma3-domain-specialist-merged",       # The full merged model
#     "gemma3-domain-specialist.gguf",         # The final GGUF model
#     "gemma3-domain-specialist-quant.gguf",
#     "domain_specific_chat_data.json",      # The dataset you generated
#     "cbook.pdf",                  # Your source PDF (assuming this is the name)
